...
import com.ruby.apps.studio.rubyvpn.billing.BillingClass;

import java.util.List;

/**
 * Этот класс реализует экран подписки в приложении.
 * На этом экране отображаются три варианта подписки на продукт с разной длительностью (1 месяц, 3 месяца и 1 год).
 * При нажатии на одну из карточек подписки происходит покупка соответствующей продукта через Google Billing API.
 * Также на экране присутствуют кнопки "No thanks" и "Cancel subscription details",
 * которые просто возвращают пользователя на предыдущий экран или открывают политику конфиденциальности, соответственно.
 * Класс также реализует методы для инициализации интерфейса и обработки ошибок при покупке продукта.
 */
...
public class SubscriptionActivity extends AppCompatActivity implements BillingClass.BillingErrorHandler, BillingClass.SkuDetailsListener {
    ...
    private void cardsDetails() {
        ...
        oneYearCheck.setImageResource(R.drawable.connection_time_bg);
    }
