...
/**
 * Этот класс содержит методы для работы с OVPN-файлами в приложении.
 * Он можно использовать для создания, сохранения, загрузки и обмена OVPN-профилями.
 * Методы класса включают:
 *
 *
 * humanReadableCount() - пересчитывает размер файла в байтах в удобочитаемый формат.
 *
 * saveConfigData() - записывает и сохраняет OVPN-профиль в файл.
 *
 * getFile() - создает пустой файл для OVPN-профиля.
 *
 * isExternalStorageWritable() - проверяет, доступно ли внешнее хранилище для чтения и записи.
 *
 * getDrawableResource() - возвращает идентификатор ресурса drawable.
 *
 * shareOvpnFile() - открывает окно выбора интента для обмена OVPN-профилем.
 */
